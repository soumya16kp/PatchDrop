...
4. Test locally with `npm run dev` — verify feeds load, filters work, bookmarks work. Use `npm run build && npm run preview` to verify the production build.
5. If you changed feeds, run `npm run build:feed` and commit the updated `public/feed.json` and `public/feed-health.json`. **Do not commit `dist/`** — it is generated by CI.
